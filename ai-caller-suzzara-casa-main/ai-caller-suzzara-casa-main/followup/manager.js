import { openDataDb, closeDataDb, runData, allData } from '../dataDb.js';
import { getGHLContactDetails } from '../ghl/api.js';
import { sendNonFatalSlackNotification } from '../slack/notifications.js';
import { OUTGOING_ROUTE_PREFIX } from '../config.js';
import { GOHIGHLEVEL_LOCATION_ID } from '../config.js';
import { createRobustFetch } from '../utils.js';


const FOLLOW_UP_INTERVAL_MS = 60 * 60 * 1000; // Check every hour
const OUTBOUND_CALL_ENDPOINT = `${OUTGOING_ROUTE_PREFIX}/outbound-call`;
...
async function deleteFollowUp(followUpId) {
    ...
    try {
        db = await openDataDb();
        await runData(db, 'DELETE FROM follow_ups WHERE follow_up_id = ?', [followUpId]);
...
async function checkAndProcessFollowUps() {
    ...
    for (const followUp of dueFollowUps) {
        ...
        try {
            ...
            const contactResponse = await getGHLContactDetails(GOHIGHLEVEL_LOCATION_ID, followUp.contact_id);

            // Handle new nested structure: contactResponse.contactDetails contains the actual contact info
            const contactDetails = contactResponse?.contactDetails;
            ...
            if (!contactDetails || !contactDetails.phone) {
                ...
                sendNonFatalSlackNotification(
                    notificationTitle,
                    notificationMessage,
                    { contactId: followUp.contact_id, followUpAtUtc: followUp.follow_up_at_utc }
                ).catch(console.error);
            ...
            const robustFetch = createRobustFetch('[FollowUp Manager]');
            const response = await robustFetch(OUTBOUND_CALL_ENDPOINT, {
                method: 'POST',
