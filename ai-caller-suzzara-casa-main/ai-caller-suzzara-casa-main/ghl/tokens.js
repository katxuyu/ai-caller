import { openTokenDb, closeTokenDb, runToken, getToken } from '../tokenDb.js';
...
export async function saveGoHighlevelTokens(location_id, access_token, refresh_token, expires_at = null) {
	...
	try {
		...
		const sql = `INSERT OR REPLACE INTO gohighlevel_tokens
					 ...
					 VALUES (?, ?, ?, ?)`;
		await runToken(db, sql, [location_id, access_token, refresh_token, expires_at_iso]);
...
export async function refreshGoHighlevelToken(location_id) {
	...
	try {
		...
		const robustFetch = createGHLFetch(`[GHL Token Refresh - ${location_id}]`);
