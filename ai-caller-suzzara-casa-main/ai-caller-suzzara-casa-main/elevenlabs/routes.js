    ...
    try {
      ...
      // STEP 4: Extract and validate dynamic variables with error handling
       ...
       try {
         ...
         firstName = dynamicVariables.firstName || dynamicVariables.first_name;
         fullName = dynamicVariables.fullName || dynamicVariables.full_name;
      ...
      try {
        let db = await openDataDb();
        
        try {
          const callRecord = await getData(db, 
      ...
      // STEP 8: Send concise and useful call completion notification
       try {
         ...
         if (analysis?.call_successful === 'success') {
           ...
           notificationTitle = 'Chiamata AI Parzialmente Riuscita';
           notificationEmoji = '⚠️';
         } else {
      ...
      // STEP 9: Log processing completion
