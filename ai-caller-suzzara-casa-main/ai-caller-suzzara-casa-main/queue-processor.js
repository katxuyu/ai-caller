...
import { openDataDb, closeDataDb, runData, getData as getDbRecord, allData as getAllDbRecords } from './dataDb.js';
...
import { GOHIGHLEVEL_LOCATION_ID, ITALIAN_TIMEZONE, PUBLIC_URL } from './config.js';
...
async function addCallAttemptNote(contactId, attemptNumber, phoneNumber = 'call_execution') {
    if (!contactId) {
...
async function processCallQueue(twilioClient) {
    ...
    try {
        ...
        // Get calls ready to be processed (oldest first), limited by MAX_ACTIVE
        ...
        if (pendingCalls.length === 0) {
            ...
            await closeDataDb(db); // Close DB if no calls found
            return;
        }

        console.log(`[Queue Processor] Found ${pendingCalls.length} calls to process (Max batch size: ${MAX_ACTIVE}).`);
        ...
        for (const callJob of pendingCalls) {
            ...
            try {
                ...
                if (!goHighLevelToken) {
                    const errorMessage = `No GoHighLevel tokens found for location ${GOHIGHLEVEL_LOCATION_ID}. Cannot process queued call for contact ${callJob.contact_id} (${callJob.full_name || callJob.first_name || 'Unknown'}).`;
            ...
            try {
                ...
                await runData(db, `DELETE FROM call_queue WHERE queue_id = ?`, [callJob.queue_id]);
