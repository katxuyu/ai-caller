...
export async function refreshGoHighlevelToken(location_id) {
	...
	try {
		...
		if (response.ok) { // Check status code 200-299
			...
			if ([400, 401].includes(response.status)) {
				...
				await sendNonFatalSlackNotification(
					'GHL Token: Refresh Failed - Re-auth Required',
					`GoHighLevel token refresh failed with status ${response.status}. Re-authorization required for location ${location_id}. This will prevent workflow triggers and API operations.`,
					{
				...
				await sendNonFatalSlackNotification(
					...
					`GoHighLevel token refresh failed with status ${response.status}. This may prevent workflow triggers and API operations.`,
					{
						locationId: location_id,
						status: response.status,
						response: response_text
		...
		await sendNonFatalSlackNotification(
			'GHL Token: Refresh Exception',
			`Unexpected error during GoHighLevel token refresh for location ${location_id}. This may prevent workflow triggers and API operations.`,
			{
				locationId: location_id,
				error: error.message,
				stack: error.stack
