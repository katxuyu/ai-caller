...
async function addCallAttemptNote(contactId, attemptNumber, phoneNumber, reason = 'call_execution') {
    if (!contactId) {
...
async function countActiveTwilioCalls(twilioClient) {
    if (!twilioClient) {
        ...
        await sendNonFatalSlackNotification(
            'Queue Processor: Missing Twilio Client',
            'countActiveTwilioCalls called without a valid Twilio client. Call processing may be impaired.',
            { critical: true }
        ).catch(console.error);
    ...
    try {
        ...
        if (error.message.includes("username is required") || error.message.includes("authenticate")) {
             ...
             await sendNonFatalSlackNotification(
                'Queue Processor: Twilio Authentication Failed',
                'Twilio authentication error suggests SID/Token might be missing or invalid. Call processing is impaired.',
                {
                    error: error.message,
                    stack: error.stack,
                    critical: true
            ...
            await sendNonFatalSlackNotification(
                ...
                'Error fetching active calls from Twilio API. Call processing may be affected.',
                {
                    error: error.message,
                    stack: error.stack
                }
...
async function processCallQueue(twilioClient) {
    if (!twilioClient) {
        ...
        await sendNonFatalSlackNotification(
            'Queue Processor: Missing Twilio Client for Processing',
            'processCallQueue called without a valid Twilio client. No calls can be processed.',
            { critical: true }
        ).catch(console.error);
        return; 
    }
    ...
    try {
        ...
        for (const callJob of pendingCalls) {
            ...
            try {
                ...
                await sendNonFatalSlackNotification(
                    'Queue Processor: GHL Token Check Failed',
                    `Failed to check GoHighLevel token for job ${callJob.queue_id}. Job skipped.`,
                    {
                        queueId: callJob.queue_id,
                        contactId: callJob.contact_id,
                        error: tokenCheckError.message,
                        stack: tokenCheckError.stack
                    }
                ).catch(console.error);
            ...
            try {
                 ...
                 await sendNonFatalSlackNotification(
                    'Queue Processor: DB Update Failed',
                    `Failed to mark job ${callJob.queue_id} as processing. Job skipped to prevent corruption.`,
                    {
                        queueId: callJob.queue_id,
                        contactId: callJob.contact_id,
            ...
            try {
                ...
                console.log(`[QUEUE PROCESSOR] Stored initial call data for SID ${callRecord.sid} with retry_count: ${callJob.retry_stage} and first_attempt_timestamp: ${callJob.first_attempt_timestamp}`);

                // Add call execution note to GoHighLevel (moved after call data storage to prevent race condition)
                await addCallAttemptNote(callJob.contact_id, callJob.retry_stage, callJob.phone_number, 'call_execution');
                ...
                await sendNonFatalSlackNotification(
                    'Queue Processor: Call Initiation Failed',
                    `Failed to initiate outbound call for job ${callJob.queue_id} to ${callJob.phone_number}. Customer contact attempt lost.`,
                    {
                        queueId: callJob.queue_id,
                ...
                try {
                    await run(db,
                     ...
                     await sendNonFatalSlackNotification(
                        'Queue Processor: DB Update Failed',
                        `Failed to update queue job status after call failure. Database consistency may be compromised.`,
                        {
                            queueId: callJob.queue_id,
                            error: failUpdateError.message,
                            stack: failUpdateError.stack
                        }
                    ).catch(console.error);
                }
            }
        ...
        await sendNonFatalSlackNotification(
            'Queue Processor: Processing Cycle Failed',
            'Critical error during queue processing cycle. Outbound call processing may be halted.',
            {
                error: error.message,
                stack: error.stack,
                critical: true
