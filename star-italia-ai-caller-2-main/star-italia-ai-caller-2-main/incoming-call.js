  ...
  for (const [key, value] of Object.entries(requiredEnvVars)) {
    if (!value) {
      ...
      sendNonFatalSlackNotification(
        'Incoming Call: Missing Environment Variable',
        `Missing required environment variable for inbound calls: ${key}. Incoming call system cannot function properly.`,
        { missingVariable: key, critical: true }
      ).catch(console.error);
  ...
  fastify.post(`${INCOMING_ROUTE_PREFIX}/incoming-call`, async (request, reply) => {
    ...
    if (!twilioCallSid || !callerNumber || !calledNumber) {
      ...
      await sendNonFatalSlackNotification(
        'Incoming Call: Missing Required Data',
        'Incoming call received with missing CallSid, From, or To data. Call will be rejected.',
        {
          requestBody: request.body,
          missingCallSid: !twilioCallSid,
          missingFrom: !callerNumber,
          missingTo: !calledNumber
        }
      ).catch(console.error);
    ...
    try {
      // Simple message for inbound calls
      let availableSlotsString = "Thank you for calling! Our team will contact you shortly to discuss your needs.";
      let slotsCount = 0;
      ...
      try {
        ...
        await sendNonFatalSlackNotification(
          'Incoming Call: ElevenLabs URL Failure',
          `Failed to get ElevenLabs signed URL for incoming call ${twilioCallSid}. Call will be terminated.`,
          {
            twilioCallSid,
      ...
      try {
        ...
        await sendNonFatalSlackNotification(
          'Incoming Call: Database Storage Failed',
          `Failed to store initial call data for incoming call ${twilioCallSid}. Call processing may be impaired.`,
          {
            twilioCallSid,
      ...
      const setupElevenLabsConnection = async () => {
        ...
        try {
          ...
          if (!callData) {
            ...
            await sendNonFatalSlackNotification(
              'Incoming Call: Critical Setup Failure',
              `No call data found in database for incoming call. Call will be terminated and customer experience compromised.`,
              {
                twilioCallSid: connectionState.twilioCallSid,
                critical: true
              }
            ).catch(console.error);
            ws.close();
          ...
          if (!signedUrlToUse) {
             ...
             try {
                ...
                await sendNonFatalSlackNotification(
                  'Incoming Call: ElevenLabs URL Failure',
                  `Failed to fetch ElevenLabs signed URL for incoming call. Call will be terminated.`,
                  {
                    twilioCallSid: connectionState.twilioCallSid,
          ...
          connectionState.elevenLabsWs.on("message", async (data) => {
            try {
              ...
              console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Received message type: ${message.type}`);
                  ...
                  if (connectionState.streamSid && payload) {
                    ...
                    try {
                      ...
                      if (ws.readyState === ws.OPEN) {
                        ...
                        console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Sent audio chunk (${payload.length} chars)`);
                      } else {
                        console.warn(`[ElevenLabs ${connectionState.twilioCallSid}] Cannot send audio - WebSocket state: ${ws.readyState}`);
                      }
                    } catch (sendError) {
                      console.error(`[ElevenLabs ${connectionState.twilioCallSid}] Failed to send audio data to Twilio:`, sendError);
                    }
                  } else {
                    console.warn(`[ElevenLabs ${connectionState.twilioCallSid}] No audio payload or streamSid missing`);
                  }
                  break;
                case "interruption":
                  console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Received interruption event. Checking if clear is needed.`);
                  // Only send clear if we're actually in the middle of audio playback
                  if (connectionState.streamSid && ws.readyState === ws.OPEN) {
                      console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Sending clear to Twilio due to interruption.`);
                      ws.send(JSON.stringify({ event: "clear", streamSid: connectionState.streamSid }));
                  } else {
                    console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Skipping clear - no active stream or WebSocket not open`);
                  }
                  break;
                case "ping":
                  if (message.ping_event?.event_id && connectionState.elevenLabsWs.readyState === WebSocket.OPEN) {
                    ...
                    console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Sent pong response`);
                  }
                  break;
                case "agent_response":
                  console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Agent response received`);
                  break;
                case "user_transcript":
                  console.log(`[ElevenLabs ${connectionState.twilioCallSid}] User transcript: ${message.user_transcript_event?.user_transcript}`);
                  break;
                case "agent_response_correction":
                  console.log(`[ElevenLabs ${connectionState.twilioCallSid}] Agent response correction received`);
                  break;
                default:
          ...
          await sendNonFatalSlackNotification(
            'Incoming Call: ElevenLabs Setup Error',
            `Critical error during ElevenLabs setup for incoming call. Customer call experience will be impacted.`,
            {
              twilioCallSid: connectionState.twilioCallSid,
              error: error.message,
              stack: error.stack,
        ...
        try {
          ...
          switch (msg.event) {
            ...
            case "stop":
              ...
              // Add a delay before sending conversation_end to ensure all audio is processed
              setTimeout(() => {
                if (connectionState.elevenLabsWs?.readyState === WebSocket.OPEN) {
                  console.log(`[TWILIO MEDIA ${connectionState.twilioCallSid}] Sending conversation_end to ElevenLabs`);
