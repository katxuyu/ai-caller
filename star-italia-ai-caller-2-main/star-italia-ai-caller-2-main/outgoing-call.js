...
export function OutgoingCall(fastify) {
  ...
  async function scheduleRetry(callDataForRetryLogic, failedCallSid, options = {}) {
    ...
    try {
        ...
        const result = await run(dbRetry,
            `INSERT INTO call_queue (contact_id, phone_number, first_name, full_name, email, retry_stage, status, scheduled_at, call_options_json, initial_signed_url, first_attempt_timestamp)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
            [baseDataForRetry.contactId, baseDataForRetry.to, baseDataForRetry.firstName, baseDataForRetry.fullName, baseDataForRetry.email, nextAttemptNumberForDB, 'pending', scheduled_at_iso, newCallOptionsJson, baseDataForRetry.initialSignedUrl, firstAttemptTimestamp.toISOString()]
        );
  ...
  fastify.post(`${routePrefix}/outbound-call`, async (request, reply) => {
    ...
    const { phone, contact_id, first_name, full_name, email } = request.body;
    ...
    console.log("[OUTBOUND CALL] Extracted parameters:", { phone, contact_id, first_name, full_name, email });
    ...
    try {
      ...
      const callOptions = {
        from: OUTGOING_TWILIO_PHONE_NUMBER,
        to: toPhoneValue,
        url: `${PUBLIC_URL}${routePrefix}/outbound-call-twiml?firstName=${encodeURIComponent(firstName || '')}&fullName=${encodeURIComponent(fullName || '')}&email=${encodeURIComponent(emailValue || '')}&phone=${encodeURIComponent(toPhoneValue)}&contactId=${encodeURIComponent(contactId)}`,
        timeout: 25,
        timeLimit: 900,
        statusCallback: `${PUBLIC_URL}${routePrefix}/call-status`,
        statusCallbackEvent: ["initiated", "ringing", "answered", "completed"],
      ...
      const result = await run(db,
         `INSERT INTO call_queue (contact_id, phone_number, first_name, full_name, email, retry_stage, status, scheduled_at, call_options_json, initial_signed_url, first_attempt_timestamp)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
         [contactId, toPhoneValue, firstName, fullName, emailValue,
  ...
  fastify.post(`${routePrefix}/call-status`, async (request, reply) => {
    ...
    if (!call) {
        ...
        await sendNonFatalSlackNotification(
          ...
          `Call status received for unknown CallSid ${CallSid}. This may indicate data consistency issues.`,
  ...
  fastify.all(`${routePrefix}/outbound-call-twiml`, async (request, reply) => {
    ...
    console.log("[TWIML] Processed parameters after XML escape:", { firstName, fullName, email, phone, contactId });
